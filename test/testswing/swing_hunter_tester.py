...
from testswing.levels_builder import build_levels, _is_mitigated
...
# Порог "это тот же уровень или новый" для паспорта уровня — то же значение,
# что LEVEL_DEDUP_TOLERANCE_PCT (bounce_manager.py) и MACRO_MERGE_DISTANCE_PCT
# (levels_builder.py) для той же по смыслу задачи.
LEVEL_PASSPORT_TOLERANCE_PCT = 4.0


def _is_poc_zone(zone):
    """POC-зона (4h_poc_standalone) — живой, скользящий показатель, не
    исторический свинг с точкой рождения (см. levels_builder.py — у неё
    'date' всегда ставится как сегодняшняя дата скана). Паспорт её не
    трогает — точное совпадение типа, а не подстрока, чтобы не задеть
    структурные зоны с POC-confluence бонусом (тип вида 'X+4h_poc')."""
...
def _reconcile_levels_with_registry(old_zones, fresh_zones, is_support, df_1d, current_idx=None):
    """Паспорт уровня — та же логика, что в боевом modules/cryptano/swing_hunter.py.

    min/max/date уже существующей записи считаются правдой по геометрии и
    НЕ переписываются свежим пересчётом (ATR гуляет от скана к скану даже
    когда сам уровень не сдвинулся). Обновляются только type/score/
    reaction_count/mitigated/class.
    ...
    только если _is_mitigated() говорит, что она реально пробита закрытием.

    POC-зоны (см. _is_poc_zone) в паспорт не попадают вообще — проходят
    как есть, свежими каждый раз."""
    ...
    for fresh in fresh_zones:
        ...
        if match_i is not None:
            ...
            merged = dict(old_zones_for_match[match_i])
...
def _reconcile_coin_levels(coin, fresh_levels, macro_base, df_1d):
    """Обёртка над _reconcile_levels_with_registry для одной монеты."""
...
def build_macro_levels(bot=None, admin_chat_id=None, target_time_str=None,
                        cache=None, valid_symbols=None, old_macro_base=None):
    ...
    old_macro_base — необязательно: результат предыдущего вызова (предыдущей
    даты по таймлайну). Если передан, свежие зоны сверяются с ним через
    паспорт (_reconcile_coin_levels) вместо того, чтобы строиться с нуля
    каждый раз — так тестер видит ту же стабильность геометрии, что и
    боевой код после этой же правки. Не передан (по умолчанию) — старое
    поведение, ничего не меняется, обратная совместимость сохранена."""

...
class VRedTopWatcher:
    ...
    def _record_event(self, event_type, price):
        ...
        Копия паттерна из v_bottom_watcher.py — время конвертируется в
    ...
    def on_breach_start(self):
        ...
        if self.state in ("DEAD", "TRIGGERED", "WAIT_C1", "WAIT_REACTION", "WAIT_C3"):
            return

        # 2. Если мы в процессе ожидания пампа и уже зафиксировали хотя бы один пик — не сбрасываем структуру
        if self.state == "WAIT_PUMP" and self.peaks_count > 0:
            return

        # 3. Полная инициализация (только при самом первом старте или после IDLE)
        self.state = "WAIT_PUMP"
        self.c1 = None
        self.c2 = None
        self.trades_count = 0
            ...
            if is_red:
                ...
                if "RED 3" in self.active_routes:
                    if body_pct >= self.CONFIG['RED3_C3_MIN_BODY_PCT'] and bottom_shadow_pct <= self.CONFIG['RED3_C3_MAX_BOTTOM_SHADOW_PCT']:
                        self.route = "RED 3"
                        self._record_event("GOOD_RED", float(c_close))
    ...
    def _enter(self, c_high, c_close, all_opposite_levels, c_rsi=0.0):
        ...
        if err or not risk_data:
            self.state = "WAIT_C1"  # ВМЕСТО "DEAD"
            self.c1 = None
            self.c2 = None
            self._record_event("CANCEL", actual_entry) # ВМЕСТО "DEAD"
            self._dbg(f"❌ Калькулятор УБИЛ сделку: {err}. Возврат в поиск.")
            return {'error': err}

...
# _watcher_lock защищает сами данные (bounce_mgr/v_bottom_mgr._watchers) —
# держать его нужно ТОЛЬКО на время реальной мутации/чтения, поэтому после
# слияния run_web.py + дашборда в один процесс (см. run_web.py) захватывается
# теперь по каждой монете отдельно внутри скан-каскада (background_tasks.py),
# а не на весь проход по ~100+ монетам разом — иначе дашборд (рескан/удаление/
# список "в работе") завис бы на всё время одного скан-цикла (минуты), ожидая
# лок, который скан держал бы не отпуская.
_watcher_lock = threading.Lock()

# _cascade_lock — отдельная блокировка, ТОЛЬКО чтобы не запустить второй
# полный скан-каскад поверх ещё не завершившегося первого (например, если
# сам каскад из-за большого количества монет не уложился в 15 минут до
# следующей плановой границы). Раньше эту роль случайно выполнял сам
# _watcher_lock (acquire(blocking=False) на весь каскад) — с переходом на
# per-coin захват _watcher_lock он для этой цели больше не подходит (он
# почти всегда свободен между монетами), поэтому нужен отдельный, узкий
# по смыслу лок.

...
class PanicTrapWatcher:
    ...
    def _advance_to_next_leg(self):
        """Изолированная функция Трекера: фиксация объемов и переход к новой ноге."""
    ...
    def _is_v_bottom_triggered(self, c_close, c_vol):
        """Изолированный Снайпер: принимает решение только о входе."""
    ...
    def update(self, c_open, c_high, c_low, c_close, c_vol, baseline_vol, all_opposite_levels, trend='UNKNOWN', **kwargs):
        ...
        # --- ГЛОБАЛЬНЫЙ СБРОС ---
        # Защищаем буфер ожидания (WAIT_NEXT_LEG) от амнезии при касании уровня
        ...
        if self.trade_type == 'LONG':
            ...
            # =================================================================
                ...
                # 1. Синхронизация: если яма углубляется
                ...
                if current_body < self.climax_body:
                    self.climax_body = current_body
                    self.climax_top = max(c_open, c_close) 
                    self.current_leg_vol = c_vol 

                # 2. Проверка отскока
                bounce_pct = ((c_close - self.climax_extreme) / self.climax_extreme) * 100

                if bounce_pct >= self.CONFIG['MIN_BOUNCE_PCT'] and c_close > c_open:
                    if self.legs_count <= 2:
                        # Трекер: Уходим в буфер памяти (ждем следующего пролива)
                        self.state = "WAIT_NEXT_LEG"
                        self.bars_since_climax = 0 
                        self._dbg(f"🟡 ОЖИДАНИЕ: Отскок есть, ждем пробой для следующей ноги (ног:{self.legs_count}).")
                        return None
                    else:
                        # Снайпер: Оцениваем вход
                        if self._is_v_bottom_triggered(c_close, c_vol):
                            self.state = "TRIGGERED"
                            # Безопасное приведение типа, чтобы Pylance/Mypy не ругались на None
                            safe_climax = float(self.climax_extreme) if self.climax_extreme is not None else c_low
                            self.sl_price = safe_climax * 0.998
                ...
                # 3. Слом текущей ноги вниз (начало новой ямы)
                if c_close < self.climax_extreme:
            ...
            # =================================================================
                ...
                # Если цена пробивает предыдущее дно, Трекер начинает писать новую ногу
                if c_low < self.climax_extreme:
                    self.state = "OBSERVE_BOUNCE"
                    self.climax_extreme = c_low
                    self.max_prev_leg_vol = max(self.max_prev_leg_vol, self.current_leg_vol)
                    
                    # --- ИСПРАВЛЕНИЕ: ЖЕСТКАЯ ПЕРЕЗАПИСЬ БАЗЫ И ОБЪЕМА ДЛЯ НОВОЙ НОГИ ---
                    self.climax_body = min(c_open, c_close)
                    self.climax_top = max(c_open, c_close)
                    self.current_leg_vol = c_vol 
                    # --------------------------------------------------------------------

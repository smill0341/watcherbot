...
from modules.cryptano.common import calculate_rsi, exchange, get_top_100_coins
    ...
    try:
        ...
        for setup in setups:
            ...
            header = (
                ...
                f"• Trend: `{setup['trend']}`\n"
                f"• Position: `{setup['pos_pct']:.0f}%` (Диапазон 30 свечей)\n"
                f"• Условие: `{setup['setup_info']}`\n"
                f"• Волатильность: `x{setup['vol_ratio']:.1f}` | RSI: `{setup['rsi']:.1f}`\n"
                f"🤖 _Запущен глубокий анализ AI Снайпера..._\n"
                f"━━━━━━━━━━━━━━━\n\n"
            )
            # Вызываем детальный анализ из analyzer.py уже после быстрого параллельного фильтра.
            analyzer_report = analyze_coin(coin_name)
            bot.send_message(admin_chat_id, header + analyzer_report, parse_mode="Markdown")
        ...
        try:
            ...
            for setup in setups:
                ...
                if market_regime == "EXTREME_PUMP":
                    ...
                    # Тяжелый анализатор запускаем только для уже отфильтрованных монет.
                    header = (
                        f"🟡 *WATCH SIGNAL | {coin_name}*\n"
                        f"• Trend: `{setup['trend']}`\n"
                        f"• Position: `{setup['pos_pct']:.0f}%` (Диапазон 30 свечей)\n"
